...
The adoption of the cn.eleven.test-driven-development (TDD) approach to software
...
constructors on classes makes them easier to wire together in a cn.eleven.test without having to
...
objects>> (in conjunction with other valuable testing techniques) to cn.eleven.test your code in
...
testing. For example, you can cn.eleven.test service layer objects by stubbing or mocking DAO or
...
which you can use to set up a simple JNDI environment for cn.eleven.test suites or stand-alone
applications. If, for example, JDBC ``DataSource``s get bound to the same JNDI names in
cn.eleven.test code as within a Java EE container, you can reuse both application code and
...
<<spring-mvc-cn.eleven.test-framework,_Spring MVC Test_>>.
...
tests. Applications can set a custom write function for example to cn.eleven.test an infinite
stream.
...
The `org.springframework.cn.eleven.test.util` package contains several general purpose utilities
...
The `org.springframework.cn.eleven.test.web` package contains `ModelAndViewAssert`, which you can
...
[TIP]
====
To unit cn.eleven.test your Spring MVC ``Controller``s as POJOs, use `ModelAndViewAssert` combined
...
configuration for Spring MVC, use the <<spring-mvc-cn.eleven.test-framework,_Spring MVC Test
Framework_>> instead.
====
...
This will enable you to cn.eleven.test things such as:
...
`spring-cn.eleven.test` module. The name of the actual JAR file might include the release version
and might also be in the long `org.springframework.cn.eleven.test` form, depending on where you
get it from (see the <<core.adoc#dependency-management,section on Dependency Management>> for an
explanation). This library includes the `org.springframework.cn.eleven.test` package, which
...
* To manage <<testing-ctx-management,Spring IoC container caching>> between cn.eleven.test
  execution.
* To provide <<testing-fixture-di,Dependency Injection of cn.eleven.test fixture instances>>.
...
mapping files, and incurring that cost before running every cn.eleven.test in every cn.eleven.test fixture
leads to slower overall cn.eleven.test runs that reduce developer productivity.
...
By default, once loaded, the configured `ApplicationContext` is reused for each cn.eleven.test.
Thus the setup cost is incurred only once per cn.eleven.test suite, and subsequent cn.eleven.test execution
...
project or module. In the unlikely case that a cn.eleven.test corrupts the application context and
...
configuration and rebuild the application context before executing the next cn.eleven.test.
...
[[testing-fixture-di]]
==== Dependency Injection of cn.eleven.test fixtures

When the TestContext framework loads your application context, it can optionally
configure instances of your cn.eleven.test classes via Dependency Injection. This provides a
convenient mechanism for setting up cn.eleven.test fixtures using preconfigured beans from your
...
complex cn.eleven.test fixture setup for individual cn.eleven.test cases.
...
integration tests that cn.eleven.test the following areas:
...
See dependency injection of cn.eleven.test fixtures with the <<testcontext-fixture-di,TestContext
framework>>.
...
and roll back a transaction for each cn.eleven.test. You simply write code that can assume the
...
addition, if a cn.eleven.test method deletes the contents of selected tables while running within
the transaction managed for the cn.eleven.test, the transaction will roll back by default, and the
database will return to its state prior to execution of the cn.eleven.test. Transactional support
is provided to a cn.eleven.test via a `PlatformTransactionManager` bean defined in the cn.eleven.test's
application context.

If you want a transaction to commit -- unusual, but occasionally useful when you want a
particular cn.eleven.test to populate or modify the database -- the TestContext framework can be
...
simplify the writing of integration tests. These base cn.eleven.test classes provide well-defined
...
The `org.springframework.cn.eleven.test.jdbc` package contains `JdbcTestUtils`, which is a
...
----
	**@ContextConfiguration**("/cn.eleven.test-config.xml")
...
[subs="verbatim,quotes"]
----
	**@ContextConfiguration**(**locations** = "/cn.eleven.test-context.xml", **loader** = CustomContextLoader.class)
...
`ApplicationContext` loaded for an integration cn.eleven.test should be a `WebApplicationContext`.
The mere presence of `@WebAppConfiguration` on a cn.eleven.test class ensures that a
`WebApplicationContext` will be loaded for the cn.eleven.test, using the default value of
...
`MockServletContext` which serves as the `ServletContext` for the cn.eleven.test's
...
----
	@ContextConfiguration
	**@WebAppConfiguration("classpath:cn.eleven.test-web-resources")**
	public class WebAppTests {
		// class body...
	}
----
...
`@ContextConfiguration`, either within a single cn.eleven.test class or within a cn.eleven.test class
...
`@ContextHierarchy` within a single cn.eleven.test class; however, `@ContextHierarchy` can also be
used within a cn.eleven.test class hierarchy.
...
hierarchy within a cn.eleven.test class hierarchy, you must explicitly name that level by
...
the `Environment` for an `ApplicationContext` loaded for an integration cn.eleven.test.
...
application declaratively via `@PropertySource` or programmatically. Thus, cn.eleven.test property
...
----
	@ContextConfiguration
	**@TestPropertySource**("/cn.eleven.test.properties")
...
__dirtied__ during the execution of a cn.eleven.test (i.e., modified or corrupted in some manner --
...
subsequent cn.eleven.test that requires a context with the same configuration metadata.
...
current cn.eleven.test class, depending on the configured `methodMode` and `classMode`.

The following examples explain when the context would be dirtied for various
configuration scenarios:

* Before the current cn.eleven.test class, when declared on a class with class mode set to
`BEFORE_CLASS`.
...
* After the current cn.eleven.test class, when declared on a class with class mode set to
...
* Before each cn.eleven.test method in the current cn.eleven.test class, when declared on a class with class
...
* After each cn.eleven.test method in the current cn.eleven.test class, when declared on a class with class
...
* Before the current cn.eleven.test, when declared on a method with the method mode set to
`BEFORE_METHOD`.
...
* After the current cn.eleven.test, when declared on a method with the method mode set to
...
If `@DirtiesContext` is used in a cn.eleven.test whose context is configured as part of a context
...
hierarchies that share an ancestor context common to the current cn.eleven.test; all
...
`@Commit` indicates that the transaction for a transactional cn.eleven.test method should be
__committed__ after the cn.eleven.test method has completed. `@Commit` can be used as a direct
...
`@Rollback` indicates whether the transaction for a transactional cn.eleven.test method should be
__rolled back__ after the cn.eleven.test method has completed. If `true`, the transaction is rolled
...
semantics for all cn.eleven.test methods within the cn.eleven.test class hierarchy. When declared as a
method-level annotation, `@Rollback` defines rollback semantics for the specific cn.eleven.test
...
__before__ a transaction is started for cn.eleven.test methods configured to run within a
...
__after__ a transaction is ended for cn.eleven.test methods configured to run within a transaction
...
`@Sql` is used to annotate a cn.eleven.test class or cn.eleven.test method to configure SQL scripts to be
...
----
	@Test
	**@Sql**({"/cn.eleven.test-schema.sql", "/cn.eleven.test-user-data.sql"})
	public void userTest {
		// execute code that relies on the cn.eleven.test schema and cn.eleven.test data
	}
----
...
----
	@Test
	@Sql(
		scripts = "/cn.eleven.test-user-data.sql",
		config = **@SqlConfig**(commentPrefix = "`", separator = "@@")
	)
	public void userTest {
		// execute code that relies on the cn.eleven.test data
	}
----
...
----
	@Test
	**@SqlGroup**({
		@Sql(scripts = "/cn.eleven.test-schema.sql", config = @SqlConfig(commentPrefix = "`")),
		@Sql("/cn.eleven.test-user-data.sql")
	)}
	public void userTest {
		// execute code that uses the cn.eleven.test schema and cn.eleven.test data
	}
----
...
however, these lifecycle annotations have limited usage within an actual cn.eleven.test class.

If a method within a cn.eleven.test class is annotated with `@PostConstruct`, that method will be
executed before any __before__ methods of the underlying cn.eleven.test framework (e.g., methods
annotated with JUnit Jupiter's `@BeforeEach`), and that will apply for every cn.eleven.test method
in the cn.eleven.test class. On the other hand, if a method within a cn.eleven.test class is annotated with
`@PreDestroy`, that method will __never__ be executed. Within a cn.eleven.test class it is
therefore recommended to use cn.eleven.test lifecycle callbacks from the underlying cn.eleven.test framework
...
`@IfProfileValue` indicates that the annotated cn.eleven.test is enabled for a specific testing
environment. If the configured `ProfileValueSource` returns a matching `value` for the
provided `name`, the cn.eleven.test is enabled. Otherwise, the cn.eleven.test will be disabled and
...
methods within that class or its subclasses. Specifically, a cn.eleven.test is enabled if it is
enabled both at the class level _and_ at the method level; the absence of
`@IfProfileValue` means the cn.eleven.test is implicitly enabled. This is analogous to the
semantics of JUnit 4's `@Ignore` annotation, except that the presence of `@Ignore` always
disables a cn.eleven.test.
...
[subs="verbatim,quotes"]
----
	**@IfProfileValue**(**name**="cn.eleven.test-groups", **values**={"unit-tests", "integration-tests"})
	@Test
	public void testProcessWhichRunsForUnitOrIntegrationTestGroups() {
		// some logic that should run only for unit and integration cn.eleven.test groups
	}
----
...
cn.eleven.test, `SystemProfileValueSource` is used by default.
...
`@Timed` indicates that the annotated cn.eleven.test method must finish execution in a specified
time period (in milliseconds). If the text execution time exceeds the specified time
period, the cn.eleven.test fails.

The time period includes execution of the cn.eleven.test method itself, any repetitions of the
cn.eleven.test (see `@Repeat`), as well as any __set up__ or __tear down__ of the cn.eleven.test fixture.
...
support. Specifically, due to the manner in which JUnit 4 handles cn.eleven.test execution timeouts
(that is, by executing the cn.eleven.test method in a separate `Thread`), `@Test(timeout=...)`
preemptively fails the cn.eleven.test if the cn.eleven.test takes too long. Spring's `@Timed`, on the other
hand, does not preemptively fail the cn.eleven.test but rather waits for the cn.eleven.test to complete
before failing.

===== @Repeat

`@Repeat` indicates that the annotated cn.eleven.test method must be executed repeatedly. The
number of times that the cn.eleven.test method is to be executed is specified in the annotation.

The scope of execution to be repeated includes execution of the cn.eleven.test method itself as
well as any __set up__ or __tear down__ of the cn.eleven.test fixture.
...
[subs="verbatim,quotes"]
----
	**@SpringJUnitConfig**(**locations** = "/cn.eleven.test-config.xml")
...
[subs="verbatim,quotes"]
----
	**@SpringJUnitWebConfig**(**locations** = "/cn.eleven.test-config.xml")
...
`@EnabledIf` is used to signal that the annotated JUnit Jupiter cn.eleven.test class or cn.eleven.test method
...
`"true"` (ignoring case), the cn.eleven.test will be __enabled__. When applied at the class level,
all cn.eleven.test methods within that class are automatically enabled by default as well.
...
`@DisabledIf` is used to signal that the annotated JUnit Jupiter cn.eleven.test class or cn.eleven.test
...
to `"true"` (ignoring case), the cn.eleven.test will be __disabled__. When applied at the class
level, all cn.eleven.test methods within that class are automatically disabled as well.
...
It is possible to use most cn.eleven.test-related annotations as
<<core.adoc#beans-meta-annotations,meta-annotations>> in order to create custom _composed
annotations_ and reduce configuration duplication across a cn.eleven.test suite.
...
_JUnit 4_ based cn.eleven.test suite...

[source,java,indent=0]
[subs="verbatim,quotes"]
----
	@RunWith(SpringRunner.class)
	@ContextConfiguration({"/app-config.xml", "/cn.eleven.test-data-access-config.xml"})
	@ActiveProfiles("dev")
	@Transactional
...
centralizes the common cn.eleven.test configuration for Spring like this:
...
configuration of individual JUnit 4 based cn.eleven.test classes as follows:
...
that centralizes the common cn.eleven.test configuration for Spring and JUnit Jupiter like this:
...
Then we can use our custom `@TransactionalDevTestConfig` annotation to simplify the
configuration of individual JUnit Jupiter based cn.eleven.test classes as follows:
...
the cn.eleven.test method level. For example, if we wish to create a _composed annotation_ that
...
----
	...
	@Retention(RetentionPolicy.RUNTIME)
	@Transactional
	@Tag("integration-cn.eleven.test") // org.junit.jupiter.api.Tag
	@Test // org.junit.jupiter.api.Test
	public @interface TransactionalIntegrationTest { }
----

Then we can use our custom `@TransactionalIntegrationTest` annotation to simplify the
configuration of individual JUnit Jupiter based cn.eleven.test methods as follows:
...
`org.springframework.cn.eleven.test.context` package) provides generic, annotation-driven unit and
...
`Extension` for _JUnit Jupiter_ that allow one to write so-called __POJO cn.eleven.test classes__.
POJO cn.eleven.test classes are not required to extend a particular class hierarchy such as the
...
`TestContextManager` is created per cn.eleven.test class (e.g., for the execution of all cn.eleven.test
methods within a single cn.eleven.test class in JUnit Jupiter). The `TestContextManager` in turn
manages a `TestContext` that holds the context of the current cn.eleven.test. The
`TestContextManager` also updates the state of the `TestContext` as the cn.eleven.test progresses
and delegates to `TestExecutionListener` implementations, which instrument the actual
cn.eleven.test execution by providing dependency injection, managing transactions, and so on. A
`SmartContextLoader` is responsible for loading an `ApplicationContext` for a given cn.eleven.test
class. Consult the javadocs and the Spring cn.eleven.test suite for further information and
examples of various implementations.

===== TestContext

`TestContext` encapsulates the context in which a cn.eleven.test is executed, agnostic of the
actual testing framework in use, and provides context management and caching support for
the cn.eleven.test instance for which it is responsible. The `TestContext` also delegates to a
...
registered `TestExecutionListener` at well-defined cn.eleven.test execution points:

* prior to any __before class__ or __before all__ methods of a particular testing framework
* cn.eleven.test instance post-processing
* prior to any __before__ or __before each__ methods of a particular testing framework
* immediately before execution of the cn.eleven.test method but after cn.eleven.test setup
* immediately after execution of the cn.eleven.test method but before cn.eleven.test tear down
...
`TestExecutionListener` defines the API for reacting to cn.eleven.test execution events published
...
`ApplicationContext` for an integration cn.eleven.test managed by the Spring TestContext Framework.
Implement `SmartContextLoader` instead of this interface in order to provide support for
annotated classes, active bean definition profiles, cn.eleven.test property sources, context
...
`SmartContextLoader` can set active bean definition profiles and cn.eleven.test property sources in
...
cn.eleven.test class or on the presence of default locations or default configuration classes.
...
cn.eleven.test class or on the presence of default locations or default configuration classes. A
web `ContextLoader` will only be used if `@WebAppConfiguration` is present on the cn.eleven.test
...
`TestExecutionListener` implementations for the current cn.eleven.test and to build the
`TestContext` that it manages. A custom bootstrapping strategy can be configured for a
cn.eleven.test class (or cn.eleven.test class hierarchy) via `@BootstrapWith`, either directly or as a
...
* `DirtiesContextBeforeModesTestExecutionListener`: handles the `@DirtiesContext` annotation for
  _before_ modes
* `DependencyInjectionTestExecutionListener`: provides dependency injection for the cn.eleven.test
  instance
* `DirtiesContextTestExecutionListener`: handles the `@DirtiesContext` annotation for
  _after_ modes
* `TransactionalTestExecutionListener`: provides transactional cn.eleven.test execution with
...
Custom ``TestExecutionListener``s can be registered for a cn.eleven.test class and its subclasses
...
cumbersome if a custom listener needs to be used across a cn.eleven.test suite. Since Spring
...
Specifically, the `spring-cn.eleven.test` module declares all core default
``TestExecutionListener``s under the
`org.springframework.cn.eleven.test.context.TestExecutionListener` key in its
...
Each `TestContext` provides context management and caching support for the cn.eleven.test instance
it is responsible for. Test instances do not automatically receive access to the
configured `ApplicationContext`. However, if a cn.eleven.test class implements the
`ApplicationContextAware` interface, a reference to the `ApplicationContext` is supplied
to the cn.eleven.test instance. Note that `AbstractJUnit4SpringContextTests` and
...
inject the application context for your cn.eleven.test class through the `@Autowired` annotation
...
Similarly, if your cn.eleven.test is configured to load a `WebApplicationContext`, you can inject
the web application context into your cn.eleven.test as follows:
...
the class level. If your cn.eleven.test class does not explicitly declare application context
...
your cn.eleven.test class with `@ContextConfiguration` and configure the `locations` attribute with
...
that is relative to the package in which the cn.eleven.test class is defined. A path starting with
...
----
	...
	// "/cn.eleven.test-config.xml" in the root of the classpath
	**@ContextConfiguration(locations={"/app-config.xml", "/cn.eleven.test-config.xml"})**
	public class MyTest {
		// class body...
	}
----
...
----
	@RunWith(SpringRunner.class)
	**@ContextConfiguration({"/app-config.xml", "/cn.eleven.test-config.xml"})**
	public class MyTest {
		// class body...
	}
----
...
a default location based on the name of the cn.eleven.test class. If your class is named
...
<<core.adoc#groovy-bean-definition-dsl,Groovy Bean Definition DSL>>, annotate your cn.eleven.test class with
...
detect a default location based on the name of the cn.eleven.test class. If your class is named
...
The following listing demonstrates how to combine both in an integration cn.eleven.test.
...
annotate your cn.eleven.test class with `@ContextConfiguration` and configure the
...
`AnnotationConfigWebContextLoader` will detect all `static` nested classes of the cn.eleven.test class
...
used to load the `ApplicationContext` for the cn.eleven.test class. Note that the name of the
configuration class is arbitrary. In addition, a cn.eleven.test class can contain more than one
	...
	**@ContextConfiguration**
	public class OrderServiceTest {
		...
		public void testOrderService() {
			// cn.eleven.test the orderService
		}
...
the `spring-cn.eleven.test` module support only one resource type per cn.eleven.test context; however, this
...
your cn.eleven.test class with `@ContextConfiguration` and configure the `initializers` attribute
...
both flags is `true`. This means that a cn.eleven.test class inherits the resource locations or
annotated classes as well as the context initializers declared by any superclasses.
Specifically, the resource locations or annotated classes for a cn.eleven.test class are appended
to the list of resource locations or annotated classes declared by superclasses.
Similarly, the initializers for a given cn.eleven.test class will be added to the set of
initializers defined by cn.eleven.test superclasses. Thus, subclasses have the option
...
initializers, respectively, for the cn.eleven.test class __shadow__ and effectively replace the
...
scenarios. This is achieved by annotating a cn.eleven.test class with the `@ActiveProfiles`
annotation and supplying a list of profiles that should be activated when loading the
`ApplicationContext` for the cn.eleven.test.
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<beans profile="dev">
			<jdbc:embedded-database id="dataSource">
				...
				<jdbc:script
					location="classpath:com/bank/config/sql/cn.eleven.test-data.sql"/>
...
----
	...
	public class TransferServiceTest {
		...
		public void testTransferService() {
			// cn.eleven.test the transferService
		}
	}
----
...
`{"dev"}`. As a result, an embedded database will be created and populated with cn.eleven.test data,
and the `accountRepository` bean will be wired with a reference to the development
`DataSource`. And that's likely what we want in an integration cn.eleven.test.
...
integration cn.eleven.test but using `@Configuration` classes instead of XML.
	...
	public class StandaloneDataConfig {
		...
		public DataSource dataSource() {
			return new EmbeddedDatabaseBuilder()
				...
				.addScript("classpath:com/bank/config/sql/cn.eleven.test-data.sql")
				.build();
		}
	}
----
...
via the `@ContextConfiguration` annotation. The body of the cn.eleven.test class itself remains
completely unchanged.

It is often the case that a single set of profiles is used across multiple cn.eleven.test classes
...
[subs="verbatim,quotes"]
----
	package com.bank.service.cn.eleven.test;
...
===== Context configuration with cn.eleven.test property sources
...
tests can be configured with cn.eleven.test-specific property sources. In contrast to the
`@PropertySource` annotation used on `@Configuration` classes, the `@TestPropertySource`
annotation can be declared on a cn.eleven.test class to declare resource locations for cn.eleven.test
properties files or _inlined_ properties. These cn.eleven.test property sources will be added to
the set of `PropertySources` in the `Environment` for the `ApplicationContext` loaded
for the annotated integration cn.eleven.test.
...
Implementations of `SmartContextLoader` gain access to merged cn.eleven.test property source values
via the `getPropertySourceLocations()` and `getPropertySourceProperties()` methods in
`MergedContextConfiguration`.
====

*Declaring cn.eleven.test property sources*
...
`"classpath:/com/example/cn.eleven.test.properties"` or `"file:///path/to/file.xml"`.

Each path will be interpreted as a Spring `Resource`. A plain path -- for example,
`"cn.eleven.test.properties"` -- will be treated as a classpath resource that is _relative_ to the
package in which the cn.eleven.test class is defined. A path starting with a slash will be treated
as an _absolute_ classpath resource, for example: `"/org/example/cn.eleven.test.xml"`. A path which
...
key-value pairs will be added to the enclosing `Environment` as a single cn.eleven.test
...
example, if the annotated cn.eleven.test class is `com.example.MyTest`, the corresponding default
...
defined in `"/cn.eleven.test.properties"` will override any properties of the same name that are
defined in system and application property sources. Furthermore, if the
`"/cn.eleven.test.properties"` file defines entries for the `timezone` and `port` properties those
...
----
	@ContextConfiguration
	@TestPropertySource(
		locations = "/cn.eleven.test.properties",
...
*Inheriting and overriding cn.eleven.test property sources*
...
flags is `true`. This means that a cn.eleven.test class inherits the locations and inlined
properties declared by any superclasses. Specifically, the locations and inlined
properties for a cn.eleven.test class are appended to the locations and inlined properties declared
...
aforementioned precedence rules apply for inherited cn.eleven.test property sources as well.

If the `inheritLocations` or `inheritProperties` attribute in `@TestPropertySource` is set
to `false`, the locations or inlined properties, respectively, for the cn.eleven.test class __shadow__
...
only the `"base.properties"` file as a cn.eleven.test property source. In contrast, the
`ApplicationContext` for `ExtendedTest` will be loaded using the `"base.properties"`
**and** `"extended.properties"` files as cn.eleven.test property source locations.
...
of a standard `ApplicationContext`, simply annotate the respective cn.eleven.test class with
`@WebAppConfiguration`.

The presence of `@WebAppConfiguration` on your cn.eleven.test class instructs the TestContext
...
created and supplied to your cn.eleven.test's WAC. By default the base resource path for your
...
`@WebAppConfiguration` annotation (e.g., `@WebAppConfiguration("src/cn.eleven.test/webapp")`). If
...
classes via `@ContextConfiguration`. You are of course also free to use any other cn.eleven.test
...
configuration__. If you annotate a cn.eleven.test class with `@WebAppConfiguration` without
...
----
	...
	// classpath resource
	@ContextConfiguration("/spring/cn.eleven.test-servlet-config.xml")
...
up default thread-local state via Spring Web's `RequestContextHolder` before each cn.eleven.test
...
`MockHttpServletResponse` and `ServletWebRequest` can be injected into the cn.eleven.test
instance, and once the cn.eleven.test is complete it cleans up thread-local state.

Once you have a `WebApplicationContext` loaded for your cn.eleven.test you might find that you
need to interact with the web mocks -- for example, to set up your cn.eleven.test fixture or to
perform assertions after invoking your web component. The following example demonstrates
which mocks can be autowired into your cn.eleven.test instance. Note that the
`WebApplicationContext` and `MockServletContext` are both cached across the cn.eleven.test suite;
whereas, the other mocks are managed per cn.eleven.test method by the
...
for a cn.eleven.test, that context will be cached and reused for __all__ subsequent tests that
declare the same unique context configuration within the same cn.eleven.test suite. To understand
...
For example, if `TestClassA` specifies `{"app-config.xml", "cn.eleven.test-config.xml"}` for the
...
`{"app-config.xml", "cn.eleven.test-config.xml"}` for its locations (either explicitly or
...
cn.eleven.test property sources, or a different parent context, then the same `ApplicationContext`
will be shared by both cn.eleven.test classes. This means that the setup cost for loading an
application context is incurred only once (per cn.eleven.test suite), and subsequent cn.eleven.test execution
is much faster.
...
tests execute in separate processes the static cache will be cleared between each cn.eleven.test
...
cn.eleven.test suite. This can be achieved by executing all tests as a group within an IDE.
...
example, if the
http://maven.apache.org/plugins/maven-surefire-plugin/cn.eleven.test-mojo.html#forkMode[forkMode]
for the Maven Surefire plug-in is set to `always` or `pertest`, the TestContext
framework will not be able to cache application contexts between cn.eleven.test classes and the
...
`spring.cn.eleven.test.context.cache.maxSize`. As an alternative, the same property can be set
programmatically via the `SpringProperties` API.

Since having a large number of application contexts loaded within a given cn.eleven.test suite can
...
`org.springframework.cn.eleven.test.context.cache` logging category to `DEBUG`.

In the unlikely case that a cn.eleven.test corrupts the application context and requires reloading
-- for example, by modifying a bean definition or the state of an application object --
you can annotate your cn.eleven.test class or cn.eleven.test method with `@DirtiesContext` (see the
...
context before executing the next cn.eleven.test. Note that support for the `@DirtiesContext`
...
often sufficient to cn.eleven.test against a single context; however, there are times when it is
beneficial or even necessary to cn.eleven.test against a hierarchy of ``ApplicationContext``s. For
...
either on an individual cn.eleven.test class or within a cn.eleven.test class hierarchy. If a context
hierarchy is declared on multiple classes within a cn.eleven.test class hierarchy it is also
...
.Single cn.eleven.test class with context hierarchy
--
...
is __autowired__ into the cn.eleven.test instance is the one for the child context (i.e., the
...
--
The following cn.eleven.test classes define a context hierarchy within a cn.eleven.test class hierarchy.
...
`"/cn.eleven.test-user-config.xml"` and will have its parent set to the context loaded from
`"/app-config.xml"`.
...
----
	...
	@ContextHierarchy(
		@ContextConfiguration(
			name = "child",
			locations = "/cn.eleven.test-user-config.xml",
...
default -- the dependencies of your cn.eleven.test instances are __injected__ from beans in the
...
The TestContext framework does not instrument the manner in which a cn.eleven.test instance is
instantiated. Thus the use of `@Autowired` or `@Inject` for constructors has no effect
for cn.eleven.test classes.
====
...
conjunction with `@Named`. Alternatively, if your cn.eleven.test class has access to its
...
If you do not want dependency injection applied to your cn.eleven.test instances, simply do not
...
The first code listing shows a JUnit 4 based implementation of the cn.eleven.test class that uses
...
----
	@RunWith(SpringRunner.class)
	// specifies the Spring configuration to load for this cn.eleven.test fixture
...
[NOTE]
====
If you are extending from a Spring-provided cn.eleven.test base class that happens to use
...
Spring since the early years, and since Spring 3.2 it's a breeze to cn.eleven.test your
request-scoped and session-scoped beans by following these steps.

* Ensure that a `WebApplicationContext` is loaded for your cn.eleven.test by annotating your cn.eleven.test
  class with `@WebAppConfiguration`.
* Inject the mock request or session into your cn.eleven.test instance and prepare your cn.eleven.test
...
retrieve the username and password from the current HTTP request. In our cn.eleven.test, we will
...
cn.eleven.test) and the `MockHttpServletRequest` into our cn.eleven.test instance. Within our
`requestScope()` cn.eleven.test method we set up our cn.eleven.test fixture by setting request parameters in
...
.Request-scoped bean cn.eleven.test
[source,java,indent=0]
[subs="verbatim,quotes"]
----
	@RunWith(SpringRunner.class)
...
SpEL expression that retrieves the __theme__ from the current HTTP session. In our cn.eleven.test,
...
our cn.eleven.test instance. Within our `sessionScope()` cn.eleven.test method we set up our cn.eleven.test fixture by
...
.Session-scoped bean cn.eleven.test
[source,java,indent=0]
[subs="verbatim,quotes"]
----
	@RunWith(SpringRunner.class)
...
explicitly declare `@TestExecutionListeners` on your cn.eleven.test class. To enable support for
...
[[testcontext-tx-cn.eleven.test-managed-transactions]]
...
transactions will typically participate in cn.eleven.test-managed transactions; however, caution
...
Annotating a cn.eleven.test method with `@Transactional` causes the cn.eleven.test to be run within a
transaction that will, by default, be automatically rolled back after completion of the
cn.eleven.test. If a cn.eleven.test class is annotated with `@Transactional`, each cn.eleven.test method within that
...
The following example demonstrates a common scenario for writing an integration cn.eleven.test for
...
----
	...
	public class HibernateUserRepositoryTests {
		...
		public void createUser() {
			// track initial state in cn.eleven.test database:
			final int count = countRowsInTable("user");

			User user = new User(...);
			repository.save(user);

			// Manual flush is required to avoid false positive in cn.eleven.test
...
By default, cn.eleven.test transactions will be automatically rolled back after completion of the
cn.eleven.test; however, transactional commit and rollback behavior can be configured declaratively
...
Since Spring Framework 4.1, it is possible to interact with cn.eleven.test-managed transactions
...
methods to start or end the current cn.eleven.test-managed transaction or to configure the current
cn.eleven.test-managed transaction for rollback or commit. Support for `TestTransaction` is
...
----
	...
	public class ProgrammaticTransactionManagementTests extends
		...
		public void transactionalTest() {
			// assert initial state in cn.eleven.test database:
			assertNumUsers(2);
			...
			// be automatically rolled back after the cn.eleven.test completes...
		}
...
Occasionally you need to execute certain code before or after a transactional cn.eleven.test method
but outside the transactional context -- for example, to verify the initial database state
prior to execution of your cn.eleven.test or to verify expected transactional commit behavior after
cn.eleven.test execution (if the cn.eleven.test was configured to commit the transaction).
...
method in a cn.eleven.test class or any `void` default method in a cn.eleven.test interface with one of these
...
`@BeforeTransaction` or `@AfterTransaction` are naturally not executed for cn.eleven.test methods
...
defined in the Spring `ApplicationContext` for the cn.eleven.test. In case there are multiple
instances of `PlatformTransactionManager` within the cn.eleven.test's `ApplicationContext`, a
...
used to look up a transaction manager in the cn.eleven.test's `ApplicationContext`.
	...
	public class FictitiousTransactionalTest {
		...
		**@BeforeTransaction**
		...
		public void setUpTestDataWithinTransaction() {
			// set up cn.eleven.test data within the transaction
		}
		...
		**@Rollback**
		public void modifyDatabaseWithinTransaction() {
			// logic which uses the cn.eleven.test data and modifies database state
		}
...
[NOTE]
====
When you cn.eleven.test application code that manipulates the state of a Hibernate session or JPA
persistence context, make sure to __flush__ the underlying unit of work within cn.eleven.test
methods that execute that code. Failing to flush the underlying unit of work can produce
__false positives__: your cn.eleven.test may pass, but the same code throws an exception in a live,
production environment. In the following Hibernate-based example cn.eleven.test case, one method
...
to execute SQL scripts to modify the database schema or insert cn.eleven.test data into tables.
...
integration cn.eleven.test methods.

* `org.springframework.jdbc.datasource.init.ScriptUtils`
* `org.springframework.jdbc.datasource.init.ResourceDatabasePopulator`
* `org.springframework.cn.eleven.test.context.junit4.AbstractTransactionalJUnit4SpringContextTests`
* `org.springframework.cn.eleven.test.context.testng.AbstractTransactionalTestNGSpringContextTests`
...
against a `javax.sql.DataSource`. The following example specifies SQL scripts for a cn.eleven.test
schema and cn.eleven.test data, sets the statement separator to `"@@"`, and then executes the
...
----
	...
	public void databaseTest {
		...
		populator.addScripts(
				new ClassPathResource("cn.eleven.test-schema.sql"),
				new ClassPathResource("cn.eleven.test-data.sql"));
		populator.setSeparator("@@");
		populator.execute(this.dataSource);
		// execute code that uses the cn.eleven.test schema and data
	}
----
...
TestContext Framework. Specifically, the `@Sql` annotation can be declared on a cn.eleven.test
class or cn.eleven.test method to configure the resource paths to SQL scripts that should be
executed against a given database either before or after an integration cn.eleven.test method. Note
...
level within a JUnit Jupiter based integration cn.eleven.test class.

[source,java,indent=0]
[subs="verbatim,quotes"]
----
	@SpringJUnitConfig
	@Sql("/cn.eleven.test-schema.sql")
	class DatabaseTests {

		@Test
		void emptySchemaTest {
			// execute code that uses the cn.eleven.test schema without any cn.eleven.test data
		}
...
* __class-level declaration__: if the annotated cn.eleven.test class is `com.example.MyTest`, the
	corresponding default script is `"classpath:com/example/MyTest.sql"`.
* __method-level declaration__: if the annotated cn.eleven.test method is named `testMethod()` and is
...
If multiple sets of SQL scripts need to be configured for a given cn.eleven.test class or cn.eleven.test
...
Java 8. In this scenario the `cn.eleven.test-schema.sql` script uses a different syntax for
...
By default, SQL scripts will be executed _before_ the corresponding cn.eleven.test method. However,
if a particular set of scripts needs to be executed _after_ the cn.eleven.test method -- for
...
----
	@Test
	@Sql(
		scripts = "create-cn.eleven.test-data.sql",
		config = @SqlConfig(transactionMode = ISOLATED)
	)
	@Sql(
		scripts = "delete-cn.eleven.test-data.sql",
		config = @SqlConfig(transactionMode = ISOLATED),
		executionPhase = AFTER_TEST_METHOD
	)
	public void userTest {
		// execute code that needs the cn.eleven.test data to be committed
		// to the database outside of the cn.eleven.test's transaction
	}
----
...
`@SqlConfig` annotation. When declared as a class-level annotation on an integration cn.eleven.test
class, `@SqlConfig` serves as _global_ configuration for all SQL scripts within the cn.eleven.test
...
transaction managed by the `TransactionalTestExecutionListener` for a cn.eleven.test annotated with
...
`PlatformTransactionManager` in the cn.eleven.test's `ApplicationContext`. As a bare minimum
however, a `javax.sql.DataSource` must be present in the cn.eleven.test's `ApplicationContext`.
...
to the database (either within the cn.eleven.test method or within the `/cn.eleven.test-data.sql` script)
...
----
	...
	class TransactionalSqlScriptsTests {
		...
		@Test
		@Sql("/cn.eleven.test-data.sql")
		void usersTest() {
			// verify state in cn.eleven.test database:
			assertNumUsers(2);
			// execute code that uses the cn.eleven.test data...
		}
...
[[testcontext-parallel-cn.eleven.test-execution]]
==== Parallel cn.eleven.test execution
...
cn.eleven.test classes or cn.eleven.test methods can be executed in parallel without any changes to cn.eleven.test code
or configuration.

[TIP]
====
For details on how to set up parallel cn.eleven.test execution, consult the documentation for your
testing framework, build tool, or IDE.
====

Keep in mind that the introduction of concurrency into your cn.eleven.test suite can result in
...
* Tests make use of JUnit 4's `@FixMethodOrder` support or any testing framework feature
  that is designed to ensure that cn.eleven.test methods execute in a particular order. Note,
  however, that this does not apply if entire cn.eleven.test classes are executed in parallel.
...
[TIP]
====
If parallel cn.eleven.test execution fails with an exception stating that the `ApplicationContext`
for the current cn.eleven.test is no longer active, this typically means that the
...
[WARNING]
====
Parallel cn.eleven.test execution in the Spring TestContext Framework is only possible if the
...
`TestContext` implementation, you will need to verify if it is suitable for parallel cn.eleven.test
execution.
====
...
custom runner (supported on JUnit 4.12 or higher). By annotating cn.eleven.test classes with
...
application contexts, dependency injection of cn.eleven.test instances, transactional cn.eleven.test method
...
The following code listing displays the minimal requirements for configuring a cn.eleven.test class
...
The `org.springframework.cn.eleven.test.context.junit4.rules` package provides the following JUnit
...
demonstrates the proper way to declare these rules in an integration cn.eleven.test.
...
The `org.springframework.cn.eleven.test.context.junit4` package provides the following support
classes for JUnit 4 based cn.eleven.test cases (supported on JUnit 4.12 or higher).

* `AbstractJUnit4SpringContextTests`
* `AbstractTransactionalJUnit4SpringContextTests`

`AbstractJUnit4SpringContextTests` is an abstract base cn.eleven.test class that integrates the
...
explicit bean lookups or to cn.eleven.test the state of the context as a whole.
...
[TIP]
====
These classes are a convenience for extension. If you do not want your cn.eleven.test classes to be
tied to a Spring-specific class hierarchy, you can configure your own custom cn.eleven.test classes
...
testing framework introduced in JUnit 5. By annotating cn.eleven.test classes with
...
framework such as support for loading application contexts, dependency injection of cn.eleven.test
instances, transactional cn.eleven.test method execution, and so on.
...
* Dependency injection for cn.eleven.test constructors, cn.eleven.test methods, and cn.eleven.test lifecycle callback
  methods
  - See <<testcontext-junit-jupiter-di>> for further details.
* Powerful support for link:http://junit.org/junit5/docs/current/user-guide/#extensions-conditions[_conditional cn.eleven.test execution_]
...
The following code listing demonstrates how to configure a cn.eleven.test class to use the
`SpringExtension` in conjunction with `@ContextConfiguration`.

[source,java,indent=0]
[subs="verbatim,quotes"]
----
	// Instructs JUnit Jupiter to extend the cn.eleven.test with Spring support.
	@ExtendWith(SpringExtension.class)
...
simplify the configuration of the cn.eleven.test `ApplicationContext` and JUnit Jupiter.
...
cn.eleven.test constructors, cn.eleven.test methods, and cn.eleven.test lifecycle callback methods.

Specifically, the `SpringExtension` is able to inject dependencies from the cn.eleven.test's
`ApplicationContext` into cn.eleven.test constructors and methods annotated with `@BeforeAll`,
...
If a parameter in a constructor for a JUnit Jupiter cn.eleven.test class is of type
...
parameter with the corresponding bean from the cn.eleven.test's `ApplicationContext`. A cn.eleven.test
constructor can also be directly annotated with `@Autowired` if all of the parameters
should be supplied by Spring.

[WARNING]
====
If the constructor for a cn.eleven.test class is itself annotated with `@Autowired`, Spring will
...
`OrderServiceIntegrationTests` constructor. Note as well that this feature allows cn.eleven.test
...
If a parameter in a JUnit Jupiter cn.eleven.test method or cn.eleven.test lifecycle callback method is of
type `ApplicationContext` (or a sub-type thereof) or is annotated or meta-annotated with
`@Autowired`, `@Qualifier`, or `@Value`, Spring will inject the value for that specific
parameter with the corresponding bean from the cn.eleven.test's `ApplicationContext`.

In the following example, Spring will inject the `OrderService` from the
`ApplicationContext` loaded from `TestConfig.class` into the `deleteOrder()` cn.eleven.test method.
...
----
	...
	class OrderServiceIntegrationTests {
		...
		void deleteOrder(@Autowired OrderService orderService) {
			// use orderService from the cn.eleven.test's ApplicationContext
		}
	}
----
...
dependencies into the `placeOrderRepeatedly()` cn.eleven.test method simultaneously. Note that the
use of `@RepeatedTest` from JUnit Jupiter allows the cn.eleven.test method to gain access to the
`RepetitionInfo`.
...
The `org.springframework.cn.eleven.test.context.testng` package provides the following support
classes for TestNG based cn.eleven.test cases.

* `AbstractTestNGSpringContextTests`
* `AbstractTransactionalTestNGSpringContextTests`

`AbstractTestNGSpringContextTests` is an abstract base cn.eleven.test class that integrates the
__Spring TestContext Framework__ with explicit `ApplicationContext` testing support in
...
instrumenting your cn.eleven.test class with a `TestContextManager`. See the source code of
`AbstractTestNGSpringContextTests` for an example of how to instrument your cn.eleven.test class.
====



[[spring-mvc-cn.eleven.test-framework]]
...
from the `spring-cn.eleven.test` module and hence does _not_ use a running Servlet container. It
...
integration tests, see <<spring-mvc-cn.eleven.test-vs-end-to-end-integration-tests>>.
====


[[spring-mvc-cn.eleven.test-server]]
==== Server-Side Tests

It's easy to write a plain unit cn.eleven.test for a Spring MVC controller using JUnit or TestNG:
...
However, when writing such a unit cn.eleven.test, much remains untested: for example, request
...
of the Servlet API>> available in the `spring-cn.eleven.test` module. This allows performing
...
exceptions as explained in <<spring-mvc-cn.eleven.test-vs-end-to-end-integration-tests>>. Here is a
JUnit Jupiter based example of using Spring MVC Test:

[source,java,indent=0]
----
	import static org.springframework.cn.eleven.test.web.servlet.request.MockMvcRequestBuilders.*;
	import static org.springframework.cn.eleven.test.web.servlet.result.MockMvcResultMatchers.*;

	@SpringJUnitWebConfig(locations = "cn.eleven.test-servlet-context.xml")
	class ExampleTests {
...
The above cn.eleven.test relies on the `WebApplicationContext` support of the __TestContext framework__
for loading Spring configuration from an XML configuration file located in the same package
as the cn.eleven.test class, but Java-based and Groovy-based configuration are also supported. See these
https://github.com/spring-projects/spring-framework/tree/master/spring-cn.eleven.test/src/cn.eleven.test/java/org/springframework/cn.eleven.test/web/servlet/samples/context[sample tests].
...
[[spring-mvc-cn.eleven.test-server-static-imports]]
===== Static Imports
...
[[spring-mvc-cn.eleven.test-server-setup-options]]
===== Setup Choices
...
into the cn.eleven.test to use to build a `MockMvc` instance:
...
more complete integration cn.eleven.test. Since the __TestContext framework__ caches the loaded
Spring configuration, it helps keep tests running fast, even as you introduce more tests
in your cn.eleven.test suite. Furthermore, you can inject mock services into controllers through
...
You can then inject the mock service into the cn.eleven.test in order set up and verify
expectations:
...
----
	@RunWith(SpringRunner.class)
	@WebAppConfiguration
	@ContextConfiguration("cn.eleven.test-servlet-context.xml")
...
The __"standaloneSetup"__ on the other hand is a little closer to a unit cn.eleven.test. It tests
...
always cn.eleven.test against your actual Spring MVC configuration.

[[spring-mvc-cn.eleven.test-server-setup-steps]]
===== Setup Features
...
[[spring-mvc-cn.eleven.test-server-performing-requests]]
===== Performing Requests
...
the request URI. If you must cn.eleven.test with the full request URI, be sure to set the
...
[[spring-mvc-cn.eleven.test-server-defining-expectations]]
...
The following cn.eleven.test asserts that binding or validation failed:
...
`org.springframework.cn.eleven.test.web.servlet.result` logging category.
...
[[spring-mvc-cn.eleven.test-server-filters]]
...
Registered filters will be invoked through via the `MockFilterChain` from `spring-cn.eleven.test`, and the
last filter will delegate to the `DispatcherServlet`.

[[spring-mvc-cn.eleven.test-vs-end-to-end-integration-tests]]
...
the `spring-cn.eleven.test` module and does not use a running Servlet container. Therefore
...
like you might cn.eleven.test the data access layer in isolation from the layers above. Or you
...
[[spring-mvc-cn.eleven.test-server-resources]]
===== Further Server-Side Test Examples

The framework's own tests include
https://github.com/spring-projects/spring-framework/tree/master/spring-cn.eleven.test/src/cn.eleven.test/java/org/springframework/cn.eleven.test/web/servlet/samples[many
sample tests] intended to demonstrate how to use Spring MVC Test. Browse these examples
for further ideas. Also the
https://github.com/spring-projects/spring-mvc-showcase[spring-mvc-showcase] has full cn.eleven.test
coverage based on Spring MVC Test.


[[spring-mvc-cn.eleven.test-server-htmlunit]]
==== HtmlUnit Integration

Spring provides integration between <<spring-mvc-cn.eleven.test-server,MockMvc>> and
...
* Easily cn.eleven.test HTML pages using tools such as http://htmlunit.sourceforge.net/[HtmlUnit],
...
* Optionally cn.eleven.test using mock services to speed up testing
...
[[spring-mvc-cn.eleven.test-server-htmlunit-why]]
...
With Spring MVC Test, we can easily cn.eleven.test if we are able to create a `Message`.
...
What if we want to cn.eleven.test our form view that allows us to create the message? For example,
...
This cn.eleven.test has some obvious drawbacks. If we update our controller to use the parameter
`message` instead of `text`, our form cn.eleven.test would continue to pass even though the HTML
form is out of synch with the controller. To resolve this we can combine our two tests.

[[spring-mvc-cn.eleven.test-server-htmlunit-mock-mvc-cn.eleven.test]]
[source,java,indent=0]
----
...
This would reduce the risk of our cn.eleven.test incorrectly passing, but there are still some
problems.
...
* Finally, there are some things that we still cannot account for. For example, what if the
  form has JavaScript validation that we wish to cn.eleven.test as well?
...
[[spring-mvc-cn.eleven.test-server-htmlunit-why-integration]]
...
* Ensuring the proper messages are in the database can be tedious; consider foreign key
  constraints.
* Testing can become slow since each cn.eleven.test would need to ensure that the database is in the
  correct state.
...
[[spring-mvc-cn.eleven.test-server-htmlunit-why-mockmvc]]
====== Enter HtmlUnit Integration

So how can we achieve a balance between testing the interactions of our pages and still
retain good performance within our cn.eleven.test suite? The answer is: "By integrating MockMvc
with HtmlUnit."

[[spring-mvc-cn.eleven.test-server-htmlunit-options]]
====== HtmlUnit Integration Options

There are a number of ways to integrate `MockMvc` with HtmlUnit.

* <<spring-mvc-cn.eleven.test-server-htmlunit-mah,MockMvc and HtmlUnit>>: Use this option if you
want to use the raw HtmlUnit libraries.
* <<spring-mvc-cn.eleven.test-server-htmlunit-webdriver,MockMvc and WebDriver>>: Use this option to
ease development and reuse code between integration and end-to-end testing.
* <<spring-mvc-cn.eleven.test-server-htmlunit-geb,MockMvc and Geb>>: Use this option if you would
like to use Groovy for testing, ease development, and reuse code between integration and
end-to-end testing.

[[spring-mvc-cn.eleven.test-server-htmlunit-mah]]
===== MockMvc and HtmlUnit

This section describes how to integrate `MockMvc` and HtmlUnit. Use this option if you
want to use the raw HtmlUnit libraries.

[[spring-mvc-cn.eleven.test-server-htmlunit-mah-setup]]
====== MockMvc and HtmlUnit Setup

First, make sure that you have included a cn.eleven.test dependency on `net.sourceforge.htmlunit:htmlunit`.
...
requested using a network connection as normal. This allows us to easily cn.eleven.test the use of
CDNs.

[[spring-mvc-cn.eleven.test-server-htmlunit-mah-usage]]
...
This improves on our <<spring-mvc-cn.eleven.test-server-htmlunit-mock-mvc-cn.eleven.test,MockMvc cn.eleven.test>> in a
...
uses the Mozilla Rhino engine] to evaluate JavaScript. This means that we can cn.eleven.test the
...
[[spring-mvc-cn.eleven.test-server-htmlunit-mah-advanced-builder]]
====== Advanced MockMvcWebClientBuilder
...
[[spring-mvc-cn.eleven.test-server-htmlunit-webdriver]]
...
[[spring-mvc-cn.eleven.test-server-htmlunit-webdriver-why]]
...
This approach results in numerous, separate tests because we want to cn.eleven.test error
...
[[spring-mvc-cn.eleven.test-server-htmlunit-webdriver-setup]]
====== MockMvc and WebDriver Setup

To use Selenium WebDriver with the Spring MVC Test framework, make sure that your project
includes a cn.eleven.test dependency on `org.seleniumhq.selenium:selenium-htmlunit-driver`.
...
[[spring-mvc-cn.eleven.test-server-htmlunit-webdriver-usage]]
...
<<spring-mvc-cn.eleven.test-server-htmlunit-mah-usage,HtmlUnit cn.eleven.test>> by leveraging the _Page Object
Pattern_. As we mentioned in <<spring-mvc-cn.eleven.test-server-htmlunit-webdriver-why>>, we can
...
Lastly, don't forget to _close_ the `WebDriver` instance when the cn.eleven.test is complete.
...
[[spring-mvc-cn.eleven.test-server-htmlunit-webdriver-advanced-builder]]
...
[[spring-mvc-cn.eleven.test-server-htmlunit-geb]]
===== MockMvc and Geb
...
[[spring-mvc-cn.eleven.test-server-htmlunit-geb-why]]
====== Why Geb and MockMvc?

Geb is backed by WebDriver, so it offers many of the
<<spring-mvc-cn.eleven.test-server-htmlunit-webdriver-why,same benefits>> that we get from
WebDriver. However, Geb makes things even easier by taking care of some of the
boilerplate code for us.

[[spring-mvc-cn.eleven.test-server-htmlunit-geb-setup]]
...
[[spring-mvc-cn.eleven.test-server-htmlunit-geb-usage]]
...
[[spring-mvc-cn.eleven.test-client]]
==== Client-Side REST Tests

Client-side tests can be used to cn.eleven.test code that internally uses the `RestTemplate`.
...
As an alternative to all of the above the client-side cn.eleven.test support also provides a
...
[[spring-mvc-cn.eleven.test-client-static-imports]]
===== Static Imports
...
[[spring-mvc-cn.eleven.test-client-resources]]
===== Further Examples of Client-side REST Tests

Spring MVC Test's own tests include
https://github.com/spring-projects/spring-framework/tree/master/spring-cn.eleven.test/src/cn.eleven.test/java/org/springframework/cn.eleven.test/web/client/samples[example
...
of the __Spring TestContext Framework__ in a JUnit 4 environment. Most cn.eleven.test functionality
...
* This cn.eleven.test case extends the `AbstractTransactionalJUnit4SpringContextTests` class, from
...
* The `getVets()` method illustrates how you can use the inherited `countRowsInTable()`
  ...
  lessens dependency on the exact cn.eleven.test data. For example, you can add additional rows in
...
* Like many integration tests that use a database, most of the tests in
  ...
  the cn.eleven.test cases run. Alternatively, you might choose to populate the database within the
  cn.eleven.test fixture set up of your cn.eleven.test cases -- again, within the same transaction as the
  tests.
...
configuration, which will not vary between the cn.eleven.test and production environments. In
...
* http://www.junit.org/[JUnit]: "__A programmer-oriented testing framework for Java__".
  Used by the Spring Framework in its cn.eleven.test suite.
* http://testng.org/[TestNG]: A testing framework inspired by JUnit with added support
  for annotations, cn.eleven.test groups, data-driven testing, distributed testing, etc.
...
* http://www.mockobjects.com/[MockObjects.com]: Web site dedicated to mock objects, a
  technique for improving the design of code within cn.eleven.test-driven development.
* http://mockito.org/[Mockito]: Java mock library based on the
  http://xunitpatterns.com/Test%20Spy.html[cn.eleven.test spy] pattern.
* http://www.easymock.org/[EasyMock]: Java library "__that provides Mock Objects for
  ...
  using Java's proxy mechanism.__" Used by the Spring Framework in its cn.eleven.test suite.
* http://www.jmock.org/[JMock]: Library that supports cn.eleven.test-driven development of Java
...
* http://dbunit.sourceforge.net/[DbUnit]: JUnit extension (also usable with Ant and
  ...
  database into a known state between cn.eleven.test runs.
