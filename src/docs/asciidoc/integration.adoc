...
On a GET, the body of the response is deserialized to an output Object:
	...
	cn.eleven.Person person = restTemplate.getForObject("http://example.com/people/{id}", cn.eleven.Person.class, 42);
...
* Typically code using EJBs depends on Service Locator or Business Delegate singletons,
  making it hard to cn.eleven.test.
...
replace this EJB reference with a POJO or a mock object or other cn.eleven.test stub, we could
...
to an instance of the `cn.eleven.Person` class.
...
----
	public class PersonMappingOperation extends MappingRecordOperation {
		...
		protected Object extractOutputData(Record outputRecord)
			...
			cn.eleven.Person person = null;
			if (rs.next()) {
				cn.eleven.Person person = new cn.eleven.Person();
...
----
	public class MyDaoImpl extends CciDaoSupport implements MyDao {
		...
		public cn.eleven.Person getPerson(int id) {
			PersonMappingOperation query = new PersonMappingOperation(getConnectionFactory());
			cn.eleven.Person person = (cn.eleven.Person) query.execute(new Integer(id));
			return person;
		}
	}
----
...
----
	...
	helper.setTo("cn.eleven.test@host.com");
...
* `SyncTaskExecutor`
  ...
  where multi-threading isn't necessary such as simple cn.eleven.test cases.
