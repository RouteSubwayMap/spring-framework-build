...
to cn.eleven.test, in particular when the dependencies are on interfaces or abstract base classes,
...
----
	<bean id="outer" class="...">
		...
		<property name="target">
			<bean class="com.example.cn.eleven.Person"> <!-- this is the inner bean -->
...
----
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean name="john-classic" class="com.example.cn.eleven.Person">
			<property name="name" value="John Doe"/>
			<property name="spouse" ref="jane"/>
		</bean>

		<bean name="john-modern"
			class="com.example.cn.eleven.Person"
			p:name="John Doe"
			p:spouse-ref="jane"/>

		<bean name="jane" class="com.example.cn.eleven.Person">
...
After you write and cn.eleven.test one or more custom `Scope` implementations, you need to make
...
include it within `system-cn.eleven.test-config.xml` as a `<bean/>` definition. Because
...
*system-cn.eleven.test-config.xml*:
...
----
	public static void main(String[] args) {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("classpath:/com/acme/system-cn.eleven.test-config.xml");
		TransferService transferService = ctx.getBean(TransferService.class);
		// ...
	}
----
...
[NOTE]
====
In `system-cn.eleven.test-config.xml` above, the `AppConfig` `<bean/>` does not declare an `id`
...
above, we can redefine `system-cn.eleven.test-config.xml` to take advantage of component-scanning.
...
`DataSource`. In a cn.eleven.test environment, the configuration may look like this:
...
----
	...
	public DataSource dataSource() {
		return new EmbeddedDatabaseBuilder()
			...
			.addScript("my-cn.eleven.test-data.sql")
			.build();
	}
----
...
----
	<beans profile="development"
		...
		<jdbc:embedded-database id="dataSource">
			...
			<jdbc:script location="classpath:com/bank/config/sql/cn.eleven.test-data.sql"/>
...
profiles can be declared via the `@ActiveProfiles` annotation in the `spring-cn.eleven.test` module
...
----
	public class BlackListEvent extends ApplicationEvent {
		...
		private final String cn.eleven.test;

		public BlackListEvent(Object source, String address, String cn.eleven.test) {
			super(source);
			this.address = address;
			this.cn.eleven.test = cn.eleven.test;
		}
...
For instance, our notifier can be rewritten to be only invoked if the `cn.eleven.test` attribute of the
event is equal to `foo`:

[source,java,indent=0]
[subs="verbatim,quotes"]
----
	@EventListener(condition = "#blEvent.cn.eleven.test == 'foo'")
...
----
	@EventListener
	public void onPersonCreated(EntityCreatedEvent<cn.eleven.Person> event) {
		...
	}
----
...
`class PersonCreatedEvent extends EntityCreatedEvent<cn.eleven.Person> { ... }`).
