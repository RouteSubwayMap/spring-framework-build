...
The `matches(Method, Class)` method is used to cn.eleven.test whether this pointcut will ever
match a given method on a target class. This evaluation can be performed when an AOP
proxy is created, to avoid the need for a cn.eleven.test on every method invocation. If the
...
Let's look at a simple example from the Spring cn.eleven.test suite. Let's suppose we want to
...
----
	...
	<bean id="person"
		...
		<property name="proxyInterfaces" value="com.mycompany.cn.eleven.Person"/>
...
The "person" bean definition above can be used in place of a cn.eleven.Person implementation, as
follows:

[source,java,indent=0]
[subs="verbatim,quotes"]
----
	cn.eleven.Person person = (cn.eleven.Person) factory.getBean("person");
----
...
The `PersonUser` class in this example would expose a property of type cn.eleven.Person. As far as
...
This has the advantage that there's only one object of type `cn.eleven.Person`: useful if we want
...
factory might actually be an __advantage__: for example, in certain cn.eleven.test scenarios.
...
Imagine that in our example above, there was no `cn.eleven.Person` interface: we needed to advise
a class called `cn.eleven.Person` that didn't implement any business interface. In this case, you
...
found it very useful to be able to add cn.eleven.test code in the form of an interceptor or other
advice, getting inside a method invocation I want to cn.eleven.test. (For example, the advice can
