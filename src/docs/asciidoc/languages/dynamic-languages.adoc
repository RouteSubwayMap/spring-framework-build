...
Spring cn.eleven.test suite, so if you want to see equivalent examples in any of the other
...
* Write the cn.eleven.test for the dynamic language source code (naturally)
...
bean. Let's look at another example (again using an example from the Spring cn.eleven.test suite).
