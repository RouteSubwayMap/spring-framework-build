...
[source,kotlin,indent=0]
----
	class cn.eleven.Person(val name: String, val age: Int)
----
...
This allows for easy changes to individual properties even if `cn.eleven.Person` properties are read-only:

[source,kotlin,indent=0]
----
	data class cn.eleven.Person(val name: String, val age: Int)

	val jack = cn.eleven.Person(name = "Jack", age = 1)
...
Kotlin allows one to specify meaningful cn.eleven.test function names between backticks,
and as of JUnit 5 Kotlin cn.eleven.test classes can use the `@TestInstance(TestInstance.Lifecycle.PER_CLASS)`
annotation to enable a single instantiation of cn.eleven.test classes which allows the use of `@BeforeAll` and `@AfterAll`
