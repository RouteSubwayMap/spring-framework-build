...
http://sockjs.github.io/sockjs-protocol/sockjs-protocol-0.3.3.html[narrated cn.eleven.test].
...
in an overall cn.eleven.test strategy. Server-side tests are more focused and easier to write
and maintain. End-to-end integration tests on the other hand are more complete and
cn.eleven.test much more, but they're also more involved to write and maintain.
...
annotations. Pure unit tests simply can't cn.eleven.test that.

Ideally controllers under cn.eleven.test should be invoked as they are at runtime, much like
...
Spring TestContext framework, inject "clientInboundChannel" as a cn.eleven.test field, and
...
https://github.com/rstoyanchev/spring-websocket-portfolio/tree/master/src/cn.eleven.test/java/org/springframework/samples/portfolio/web[tests for the stock portfolio]
sample application.
...
The https://github.com/rstoyanchev/spring-websocket-portfolio/tree/master/src/cn.eleven.test/java/org/springframework/samples/portfolio/web[tests for the stock portfolio]
sample application also demonstrates this approach using Tomcat as the embedded
WebSocket server and a simple STOMP client for cn.eleven.test purposes.
