...
program that cn.eleven.test drives the above configuration:
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<context:component-scan base-package="org.springframework.docs.cn.eleven.test" />
...
the datasource. You most likely fill both roles as you develop and cn.eleven.test code, but you do
...
This is primarily a cn.eleven.test class. For example, it enables easy testing of code outside an
...
This implementation is useful for cn.eleven.test and stand-alone environments outside of a Java EE
...
using JavaBean-style connection pools such as `commons-dbcp` is so easy, even in a cn.eleven.test
...
[subs="verbatim,quotes"]
----
	final TestItem testItem = new TestItem(123L, "A cn.eleven.test item",
...
----
	<jdbc:embedded-database id="dataSource" generate-name="true">
		...
		<jdbc:script location="classpath:cn.eleven.test-data.sql"/>
	</jdbc:embedded-database>
----

The preceding configuration creates an embedded HSQL database populated with SQL from
`schema.sql` and `cn.eleven.test-data.sql` resources in the root of the classpath. In addition, as
...
standalone environment or in a standalone integration cn.eleven.test like in the following example.
...
Embedded databases provide a lightweight way to cn.eleven.test data access code. The following is a
data access integration cn.eleven.test template that uses an embedded database. Using a template
like this can be useful for _one-offs_ when the embedded database does not need to be
reused across cn.eleven.test classes. However, if you wish to create an embedded database that is
shared within a cn.eleven.test suite, consider using the <<testing.adoc#testcontext-framework,Spring TestContext
...
Development teams often encounter errors with embedded databases if their cn.eleven.test suite
...
across multiple testing scenarios within the same cn.eleven.test suite (i.e., within the same JVM
...
----
	<jdbc:initialize-database data-source="dataSource">
		...
		<jdbc:script location="classpath:com/foo/sql/db-cn.eleven.test-data.sql"/>
	</jdbc:initialize-database>
----

The example above executes the two scripts specified against the database: the first
script creates a schema, and the second populates tables with a cn.eleven.test data set. The script
...
executing the scripts against a database that already has cn.eleven.test data in it. The likelihood
...
EXISTS` (or similar) but you want to unconditionally remove all cn.eleven.test data before
...
----
	<jdbc:initialize-database data-source="dataSource" **separator="@@"**>
		...
		<jdbc:script location="classpath:com/foo/sql/db-cn.eleven.test-data-1.sql"/>
		<jdbc:script location="classpath:com/foo/sql/db-cn.eleven.test-data-2.sql"/>
	</jdbc:initialize-database>
----

In this example, the two `cn.eleven.test-data` scripts use `@@` as statement separator and only
...
* __Easier testing.__ Spring's IoC approach makes it easy to swap the implementations
  ...
  in turn makes it much easier to cn.eleven.test each piece of persistence-related code in
  isolation.
	...
	public class ProductDaoImpl implements ProductDao {
		...
		public Collection loadProductsByCategory(String category) {
			return this.sessionFactory.getCurrentSession()
					.createQuery("from cn.eleven.test.Product product where product.category=?")
...
other than JTA, transaction support also works in a stand-alone or cn.eleven.test environment.
...
tier in desktop applications or cn.eleven.test suites.
...
option is sufficient only for stand-alone applications and cn.eleven.test environments, for which
