...
the reactive <<webflux-client,WebClient>>, support for <<webflux-cn.eleven.test>>,
...
[[webflux-cn.eleven.test]]
== Testing
[.small]#<<web.adoc#testing,Same in Spring MVC>>#

The `spring-cn.eleven.test` module provides mock implementations of `ServerHttpRequest`,
